/* global $, Files, Notifier, Saves, DarkMode, ColorPicker, Updater, Api, HeroData, FlatStatCalibration */
...
const Settings = {
  initialize: async () => {
    ...
    $('#settingMaxResultsKeepN').on('keyup', formatNumbersOnKey);
    ...
    async function _uploadCacheFile(label, destFilename) {
      ...
      try {
        const text = await Files.readFileSync(Files.path(filenames[0]));
        const destPath = `${Files.getCachePath()}/cache/${destFilename}`;
        Files.saveFile(destPath, text);
        Notifier.success(
          `${label} loaded — restart or re-check updates to apply`,
        );
      } catch (e) {
        Notifier.error(`Failed to load ${label}: ${e}`);
      }
    }

    document
      .getElementById('uploadHeroDataBtn')
      .addEventListener('click', () =>
        _uploadCacheFile('Hero Data', 'herodata.json'),
      );

    document
      .getElementById('uploadArtifactDataBtn')
      .addEventListener('click', () =>
        _uploadCacheFile('Artifact Data', 'artifactdata.json'),
      );
    ...
    function _updateCalibrationStatus() {
      ...
      document.getElementById('calibrateFlatStatsResult').textContent = msg;
    }

    document
      .getElementById('calibrateFlatStatsBtn')
      .addEventListener('click', () => {
        const result = FlatStatCalibration.calibrate(HeroData.getAllHeroData());
  ...
  getUseLocalCache: () => {
    const el = document.getElementById('settingUseLocalCache');
  ...
  getDefaultSettings: () => {
    return {
      ...
      settingMaxResultsKeepN: 5_000_000,
      settingMaxRamGb: 6,
  ...
  loadSettings: async () => {
    ...
    document.getElementById('settingGpu').checked = isNullUndefined(
      ...
      ? true
      : settings.settingGpu;
    document.getElementById('settingUnlockOnUnequip').checked = isNullUndefined(
    ...
    document.getElementById('settingPenSet').checked = isNullUndefined(
      ...
      ? true
      : settings.settingPenSet;
    document.getElementById('settingFervorSet').checked = isNullUndefined(
      settings.settingFervorSet,
    )
      ? true
      : settings.settingFervorSet;
    ...
    document.getElementById('settingUseLocalCache').checked = isNullUndefined(
      settings.settingUseLocalCache,
    )
      ? false
      : settings.settingUseLocalCache;
    document.getElementById('settingKeepBestN').checked = isNullUndefined(
      settings.settingKeepBestN,
    )
      ? false
      : settings.settingKeepBestN;
    ...
    if (settings.settingMaxResultsKeepN) {
      document.getElementById('settingMaxResultsKeepN').value =
        settings.settingMaxResultsKeepN
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    } else {
      document.getElementById('settingMaxResultsKeepN').value = '5,000,000';
      settings.settingMaxResultsKeepN = 5_000_000;
    }
  ...
  saveSettings: async () => {
    ...
    const settings = {
      ...
      settingUnlockOnUnequip: document.getElementById('settingUnlockOnUnequip')
        .checked,
      ...
      settingUseLocalCache: document.getElementById('settingUseLocalCache')
        .checked,
      settingKeepBestN: document.getElementById('settingKeepBestN').checked,
      settingMaxResults: parseInt(
        Settings.parseNumberValue('settingMaxResults') || 5_000_000,
        10,
      ),
      settingMaxResultsKeepN: parseInt(
        Settings.parseNumberValue('settingMaxResultsKeepN') || 5_000_000,
        10,
      ),
      ...
      settingBackgroundColor: document.getElementById('backgroundColorPicker')
        .value,
      ...
      settingGridTextColorPicker: document.getElementById('gridTextColorPicker')
        .value,

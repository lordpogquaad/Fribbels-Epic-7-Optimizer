...
function LightenDarkenColor(col, amt) {
  ...
  // padStart keeps 6 hex digits — small results (near-zero channels) would
  // otherwise drop leading zeros and produce a malformed color.
  const hex = (g + b * 256 + r * 65536).toString(16).padStart(6, '0');
  return (usePound ? '#' : '') + hex;
}
...
globalThis.darkUserGradient = darkUserGradient;

// Rebuild the red→neutral→green grid-score gradient from the current myColors.
function _rebuildUserGradient() {
...
// Redraw every grid that shows gradient-colored cells (skips any not yet built).
function _redrawGrids() {
  globalThis.itemsGrid?.gridOptions?.api?.redrawRows();
  globalThis.buildsGrid?.gridOptions?.api?.redrawRows();
  globalThis.optimizerGrid?.gridOptions?.api?.redrawRows();
}

const ColorPicker = {
  redraw: () => {
    _rebuildUserGradient();
    _redrawGrids();
  },

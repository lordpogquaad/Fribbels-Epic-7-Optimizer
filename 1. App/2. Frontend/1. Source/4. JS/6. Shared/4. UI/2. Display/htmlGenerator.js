...
function isItemModded(item) {
  return item.substats.some((x) => x.modified);
}
...
function getPercentageEquivalent(stat, baseStats, useReforged) {
  const value = useReforged ? (stat.reforgedValue ?? stat.value) : stat.value;
...
function statToText(stat, baseStats, item, checkboxPrefix, showMaxSpeed) {
  ...
  if (Reforge.isReforgeable(item) && stat.modified && stat.originalType) {
    ...
    value = stat.type.includes('Percent')
      ? `${stat.reforgedValue ?? stat.value}%`
      : getPercentageEquivalent(stat, baseStats, item.alreadyPredictedReforge);
  } else {
  ...
  if (stat.type === 'Speed' && item.enhance !== 15 && showMaxSpeed === true) {
    ...
    if (speedStats.length > 0) {
      ...
      const potentialRolls =
        Constants.speedByItemTypeAlreadyRolled[item.rank][roundedEnhance];

      const reforgedValue =
        item.level === 90
          ? speedStat.value
          : (speedStat.reforgedValue ?? speedStat.value);
      ...
      const potentialRolls =
        Constants.speedByItemTypeNotYetRolled[item.rank][roundedEnhance];
      maxSpeed =
        potentialRolls * 4 +
        (item.level === 85 ? Constants.speedRollsToValue[potentialRolls] : 0);
    }
...
function wssToText(item) {
  if (item.equippedByName && hasFlat(item)) {
    const baseStatsEntry = HeroData.getBaseStatsByName(
      item.equippedByName.replace(/\s#\d+$/, ''),
    );
    if (!baseStatsEntry) return '';
    const baseStats = baseStatsEntry.lv60SixStarFullyAwakened;
    ...
    if (Reforge.isReforgeable(item)) {
      if (isItemModded(item) && item.substats.some((x) => x.originalType)) {
...
function getHeroImage(item) {
  ...
  const baseName = heroName.replace(/\s#\d+$/, '');
  const data = HeroData.getHeroExtraInfo(baseName);
  if (!data) return Assets.getBlank();
...
const huntImageBySet = {
  ...
  HeroSet: './assets/reforgeOtherworld.png',
};
...
function locate(item, checkboxPrefix) {
  return `<img src="${Assets.getLocate()}" class="itemDisplayEditImg locateTooltip${escAttr(checkboxPrefix)}" data-itemId="${escAttr(item.ingameId)}" onclick='Locator.locate("${escAttr(item.ingameId)}", "locateTooltip${escAttr(checkboxPrefix)}")'></img>`;
}

function storage(item, checkboxPrefix) {
  if (!item.storage) return ''; // blank transparent

  return `<img src="${Assets.getStorage()}" class="itemDisplayEditImg locateTooltip${escAttr(checkboxPrefix)}" data-itemId="${escAttr(item.ingameId)}" onclick='Locator.locate("${escAttr(item.ingameId)}", "locateTooltip${escAttr(checkboxPrefix)}")'></img>`;
}
...
function magnify(item) {
  return `<img src="${Assets.getMagnify()}" class="itemDisplayEditImg" onclick='EnhancingTab.redrawEnhanceGuideFromRemoteId("${escAttr(item.id)}")'></img>`;
}

function editItemDisplay(item, checkboxPrefix) {
  if (Reforge.isReforgeableNow(item)) {
    return `<img src="${Assets.getEdit()}"    class="itemDisplayEditImg" onclick='OptimizerTab.editGearFromIcon("${escAttr(item.id)}", false, "${escAttr(checkboxPrefix)}")'></img>
                <img src="${Assets.getReforge()}" class="itemDisplayEditImg" onclick='OptimizerTab.editGearFromIcon("${escAttr(item.id)}", true, "${escAttr(checkboxPrefix)}")'></img>`;
  }
  return `<img src="${Assets.getEdit()}" class="itemDisplayEditImg" onclick='OptimizerTab.editGearFromIcon("${escAttr(item.id)}", false, "${escAttr(checkboxPrefix)}")'></img>`;
}
...
function editLockDisplay(item, checkboxPrefix) {
  if (!item.locked) {
    return `<img src="${Assets.getLock()}" style="opacity:0.12" class="itemDisplayLockImgTransparent" onclick='OptimizerTab.lockGearFromIcon("${escAttr(item.id)}", "${escAttr(checkboxPrefix)}")'}></img>`;
  }
  return `<img src="${Assets.getLock()}" class="itemDisplayLockImg" onclick='OptimizerTab.lockGearFromIcon("${escAttr(item.id)}", "${escAttr(checkboxPrefix)}")'}></img>`;
}

function disableModsDisplay(item, checkboxPrefix) {
  if (!item.disableMods) {
    return `<img src="${Assets.getSetBlock()}" class="itemDisplayDisableModsImg" title="${i18next.t('Click to disable mods for this item')}" onclick='OptimizerTab.toggleDisableModsFromIcon("${escAttr(item.id)}", "${escAttr(checkboxPrefix)}")'></img>`;
  }
  return `<img src="${Assets.getSetBlock()}" class="itemDisplayDisableModsImgActive" title="${i18next.t('Mods disabled — click to re-enable')}" onclick='OptimizerTab.toggleDisableModsFromIcon("${escAttr(item.id)}", "${escAttr(checkboxPrefix)}")'></img>`;
}
...
function styleForImage(item) {
  const rank = typeof item === 'string' ? item : item?.rank;
...
<div class="itemDisplayFooter">
    ...
    const equippedName = item.equippedByName || '';
    const dupeMatch = equippedName.match(/\s(#\d+)$/);
    const dupeBadge = dupeMatch ? dupeMatch[1] : null;
    const heroIconHtml = dupeBadge
      ? `<div class="itemDisplayEquippedHeroWrap"><img src="${heroImage}" class="itemDisplayEquippedHero" title="${escAttr(equippedName)}"></img><span class="itemDisplayEquippedHeroBadge">${escAttr(dupeBadge)}</span></div>`
      : `<img src="${heroImage}" class="itemDisplayEquippedHero"${equippedName ? ` title="${escAttr(equippedName)}"` : ''}></img>`;
...
<div class="itemDisplayFooter">
  ...
  buildHeroModSummary(hero) {
    ...
    if (!hero?.keepStats || hero.keepStats.length === 0) return '';
    ...
    if (hero.limitRolls && hero.limitRolls < 6) {
      parts.push(
        `Limit ${hero.limitRolls} roll${hero.limitRolls === 1 ? '' : 's'}`,
      );
    }
    ...
    if (hero.modSlots && hero.modSlots.length > 0 && hero.modSlots.length < 6) {
      parts.push(
        `${hero.modSlots.map((s) => slotShort[s] || s).join('+')} only`,
      );
    }
    ...
    if (hero.slotModConfig) {
      ...
      for (const slot of [
        ...
        if (cfg?.rules && cfg.rules.length > 0) {
          const enabledCount = cfg.rules.filter(
            (r) => r.enabled !== false,
          ).length;
          if (enabledCount > 0) {

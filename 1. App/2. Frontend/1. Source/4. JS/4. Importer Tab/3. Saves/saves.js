/* global Files, Settings, Api, Dialog, Notifier, ItemAugmenter, ModificationFilter, HeroData, OptimizerTab, HeroesTab, ItemsTab, $ */
...
const Saves = {
  ...
  initialize: async () => {
    ...
    document
      ...
      .addEventListener('click', async () => {
        ...
        if (!data) return; // save failed; saveData already surfaced the error
  ...
  loadSavedData: async (data) => {
    ...
    if (data.heroes?.some((x) => x.name === 'Euglmomorain')) {
    ...
    // Remove bad overrides — strip #N duplicate suffix before checking validity
    await Promise.all(
      data.heroes
        .filter((hero) => !heroNames.includes(hero.name.replace(/\s#\d+$/, '')))

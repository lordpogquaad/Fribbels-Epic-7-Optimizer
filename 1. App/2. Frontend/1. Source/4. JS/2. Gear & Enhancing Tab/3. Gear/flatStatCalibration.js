...
function _load() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed?.atk && parsed.hp && parsed.def) {
    ...
    Log.warn('[FlatStatCalibration] Failed to load from localStorage:', e);
  }
  _cache = { ...DEFAULTS };
  return _cache;
}
...
const FlatStatCalibration = {
  ...
  calibrate(heroesByName) {
    ...
    Object.values(heroesByName).forEach((hero) => {
      ...
      const stats = hero.calculatedStatus?.lv60SixStarFullyAwakened;
      if (!stats) return;
    ...
    if (count === 0) {
      Log.warn(
        '[FlatStatCalibration] No eligible heroes found — keeping current weights.',
      );
    ...
    try {
      ...
      Log.warn('[FlatStatCalibration] Failed to save to localStorage:', e);
    }

    globalThis.dispatchEvent(
      new CustomEvent('flatStatCalibrationChanged', { detail: result }),
    );
    return result;
  },
  ...
  reset() {
    ...
    try {
      ...
      Log.warn(
        '[FlatStatCalibration] Failed to remove from localStorage:',
        e,
      );
    }
    globalThis.dispatchEvent(
      new CustomEvent('flatStatCalibrationChanged', {
        detail: { ..._cache, heroCount: null },
      }),
    );
  },
  ...
  isCalibrated() {
    try {
      ...
      Log.warn('[FlatStatCalibration] Failed to check localStorage:', e);
      return false;
    }
  },

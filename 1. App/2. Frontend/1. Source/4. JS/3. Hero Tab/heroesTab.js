...
function updateLastRankDisplay() {
  ...
  const nameSuffix = lastRankEntry.name ? ` · ${lastRankEntry.name}` : '';
  el.textContent = `Last: #${lastRankEntry.rank}${nameSuffix}`;
}

const HeroesTab = {
  initialize: () => {
    Log.debug('[HeroesTab] initialize');
    setupEventListeners();
    const selector = document.getElementById('addHeroesSelector');
    const allHeroData = HeroData.getAllHeroData();
    Log.debug(
      '[HeroesTab] allHeroData loaded, hero count:',
      Object.keys(allHeroData).length,
    );
    const names = Object.keys(allHeroData).sort((a, b) => a.localeCompare(b));

    Log.debug('[HeroesTab] hero names:', names);
    ...
    document
      ...
      .addEventListener('click', async () => {
        Log.debug('[HeroesTab] addHeroesSubmit:', selector.value);
    ...
    document
      ...
      .addEventListener('change', async () => {
        ...
        Log.debug(
          `[HeroesTab] reforgedStats changed to: ${useReforgedStats}`,
        );
        await redrawGrid();
        clearPreview();
    ...
    document
      .getElementById('heroesTabSeparateLowPick')
      ?.addEventListener('change', (e) => {
        HeroesGrid.setSeparateLowPick(e.target.checked);
      });

    document
      .getElementById('heroesTabAutoRank')
      ?.addEventListener('click', () => {
        HeroesGrid.autoRank();
      });
    ...
    document
      ...
      .addEventListener('click', async () => {
        Log.debug('[HeroesTab] editBuildSubmit');
        const row = HeroesGrid.getSelectedRow();
        if (!row) { Notifier.warn('Select a hero first.'); return; }
        ...
        Log.debug(
          '[HeroesTab] editBuildSubmit result:',
          editedBuild,
          existingBuild,
        );
    ...
    document
      ...
      .addEventListener('click', async () => {
        Log.debug('[HeroesTab] removeBuildSubmit');
        const row = HeroesGrid.getSelectedRow();
        if (!row) return;
        const existingBuild = HeroesGrid.getSelectedBuildRow();

        Log.debug(
          '[HeroesTab] removeBuildSubmit row/build:',
          row,
          existingBuild,
        );
    ...
    document
      ...
      .addEventListener('click', async () => {
        ...
        await Api.equipItemsOnHero(row.id, existingBuild.items, useReforgedStats);
        HeroesGrid.refreshBuilds();
        HeroesTab.redraw();
        HeroesGrid.redrawPreview();
        Saves.autoSave();
    ...
    document
      ...
      .addEventListener('click', async () => {
        const row = HeroesGrid.getSelectedRow();
        if (!row) return;
        Log.debug('addSubstatModsSubmit', row);
    ...
    document
      ...
      .addEventListener('click', async () => {
        const row = HeroesGrid.getSelectedRow();
        if (!row) return;
        Log.debug('setHeroRankSubmit', row);

        const rankInfo = await Dialog.editRankDialog(
          lastRankEntry.rank ?? row.index + 1,
        );
        if (!rankInfo) return;

        await Api.reorderHeroes(row.id, rankInfo.rank).then(HeroesTab.redraw);
    ...
    document
      ...
      .addEventListener('click', async () => {
        const row = HeroesGrid.getSelectedRow();
        if (!row) return;
        Log.debug('addHeroStatsSubmit', row);
    ...
    document
      ...
      .addEventListener('click', async () => {
        Log.debug('[HeroesTab] removeHeroesSubmit');
        const row = HeroesGrid.getSelectedRow();
        if (!row) return;

        const removeResponse = await Api.removeHeroById(row.id);
        Log.debug('[HeroesTab] removeHero response:', removeResponse);
    ...
    document
      ...
      .addEventListener('click', async () => {
        Log.debug('unequipHeroesSubmit', HeroesGrid.getSelectedRow());
        const row = HeroesGrid.getSelectedRow();
        if (!row) return;

        const unequipResponse = await Api.unequipItems(getSelectedGearIds());
        Log.debug('RESPONSE', unequipResponse);
        await redrawGrid();
        clearPreview();
        HeroesGrid.redrawPreview();
        Saves.autoSave();
    ...
    document
      ...
      .addEventListener('click', async () => {
        Log.debug('unlockHeroesSubmit', HeroesGrid.getSelectedRow());
        const row = HeroesGrid.getSelectedRow();
        if (!row) return;

        const unlockResponse = await Api.unlockItems(getSelectedGearIds());
        Log.debug('RESPONSE', unlockResponse);
        await redrawGrid();
        clearPreview();
        HeroesGrid.redrawPreview();
        Saves.autoSave();
    ...
    document
      ...
      .addEventListener('click', async () => {
        Log.debug('lockHeroesSubmit', HeroesGrid.getSelectedRow());
        const row = HeroesGrid.getSelectedRow();
        if (!row) return;

        const lockResponse = await Api.lockItems(getSelectedGearIds());
        Log.debug('RESPONSE', lockResponse);
        await redrawGrid();
        clearPreview();
        HeroesGrid.redrawPreview();
        Saves.autoSave();
    ...
    Api.getAllHeroes(useReforgedStats)
      .then((response) => {
        Log.debug('[HeroesTab] getAllHeroes response:', response);

        if (!response?.heroes) return null;
  ...
  doesExternalFilterPass: (rowData) => {
    ...
    if (
      filters.classFilter.length > 0 &&
      !filters.classFilter.includes(hero.role)
    ) {
      return false;
    }
    if (
      filters.elementFilter.length > 0 &&
      !filters.elementFilter.includes(hero.attribute)
    ) {
      return false;
    }
  ...
  getNewHeroByName: (heroName) => {
    ...
    const id = uuidv4();
    const data = structuredClone(heroData);
  ...
  showBonusStatsWindow: async (row) => {
    ...
    Log.debug('Bonus stats', bonusStats, row.id);
    ...
    if (imprintNumberText !== 'None') {
      const imprintNumber = Number.parseFloat(imprintNumberText);

      Log.debug('ADDING AEI IMPRINT', imprintNumber, imprintBonusType);

      if (imprintBonusType) {
        bonusStats[imprintBonusType] += imprintNumber;
      } else {
        Log.warn(`[HeroesTab] Unknown imprint stat type: ${imprintIngameType}`);
      }
    }
    ...
    if (artifactName !== 'None') {
      ...
      if (artifactLevelText !== 'None') {
        ...
        Log.debug('ADDING AEI ARTIFACT', artifactLevel);
        Log.debug('ADDING AEI ARTIFACT', artifactLevelText);
        Log.debug('ADDING AEI ARTIFACT', artifactName);
    ...
    if (eeNumberText !== 'None') {
      const eeNumber = Number.parseInt(eeNumberText, 10);
      const eeIngameType = bonusStats.ee.stat.type;
      const eeBonusType = e7StatToBonusStat[eeIngameType];

      Log.debug('ADDING AEI EE', eeBonusType, eeNumber);

      if (eeBonusType) {
        bonusStats[eeBonusType] += eeNumber;
      } else {
        Log.warn(`[HeroesTab] Unknown EE stat type: ${eeIngameType}`);
      }
    }
...
function setupEventListeners() {
  ...
  function showSearchDropdown(heroes) {
    ...
    heroes.forEach((hero) => {
      ...
      item.textContent = `${hero.label || hero.name} (Rank ${hero.index})`;
      item.addEventListener(
        'mousedown',
        onDropdownItemMousedown.bind(null, hero),
      );
  ...
  function onDropdownItemMousedown(hero, e) {
    e.preventDefault();
    const name = hero.label || hero.name;
    searchInput.value = name;
    filters.nameFilter = name;
    HeroesGrid.refreshFilters(filters);
    hideSearchDropdown();
    HeroesGrid.scrollToHero(name, hero.index);
  }
}

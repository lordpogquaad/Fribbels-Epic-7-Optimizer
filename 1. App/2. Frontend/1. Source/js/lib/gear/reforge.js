...
function isGaveleets(gear) {
    ...
    return (
        Utils.stringDistance("Gaveleet's", gear.name) > 0.45 &&
        gear.level === 85
    );
}
...
function calculateReforgeValues(substat) {
    if (plainStats.includes(substat.type)) {
        substat.reforgedValue =
            substat.value + plainStatRollsToValue[substat.rolls];
    }
    ...
    if (substat.type === 'CriticalHitDamagePercent') {
        substat.reforgedValue =
            substat.value + critDamageRollsToValue[substat.rolls];
    }
    ...
    if (substat.type === 'Speed') {
        substat.reforgedValue =
            substat.value + speedRollsToValue[substat.rolls];
    }
}
    ...
    if (rolls !== maxRolls) {
        ...
        Array.from({ length: missingRolls }).forEach(() => {
            gear.substats.forEach((substat) => {
                ...
                substat.scaledDiff =
                    (value - substat.minExpectedValue) / substat.multi;
            });
        ...
        substats.forEach((substat) => {
            ...
            if (substat.type === 'Attack') {
                substat.unreforgedMin =
                    substat.rolls *
                    flats.Attack.min *
                    modifiers.Attack.min *
                    power;
                substat.unreforgedMax =
                    substat.rolls *
                    flats.Attack.max *
                    modifiers.Attack.max *
                    power;
            }
            if (substat.type === 'Defense') {
                substat.unreforgedMin =
                    substat.rolls *
                    flats.Defense.min *
                    modifiers.Defense.min *
                    power;
                substat.unreforgedMax =
                    substat.rolls *
                    flats.Defense.max *
                    modifiers.Defense.max *
                    power;
            }
            if (substat.type === 'Health') {
                substat.unreforgedMin =
...
*/
        ...
        substats.forEach((substat) => {
            ...
            const potential =
                (reforgedValue - reforgedMin) / (reforgedMax - reforgedMin);

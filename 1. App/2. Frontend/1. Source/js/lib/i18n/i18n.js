...
export default {
    initialize: async () => {
        const dataPath = Files.getDataPath();
        const locales = ['en', 'zh', 'ko', 'zh-TW', 'fr', 'ja', 'ru'];
        for (const locale of locales) {
            window.require(
                Files.path(dataPath + '/locales/' + locale + '/gridlocale.js'),
            );
        }
        ...
        window.i18next
            ...
            .init({
                ...
                backend: {
                    ...
                    // }, {
                    loadPath: Files.path(
                        Files.getDataPath() + '/locales/{{lng}}/{{ns}}.json',
                    ),
                    addPath: Files.path(
                        Files.getDataPath() +
        ...
        window.i18next.on(
            'languageChanged initialized reloadResources',
            function () {
                        ...
                        function (currentValue, currentIndex, listObj) {
                            ...
                            if (
                                [
                                    ...
                                    'OPTION',
                                    'BUTTON',
                                ].includes(currentValue.nodeName)
                            ) {
                                ...
                                if (currentValue.getAttribute('data-content')) {
                                    const datacontent =
                                        currentValue.getAttribute(
                                            'data-content',
                                        );
                                    const trimedDatacontent =
                                        datacontent.trim();
                                    const transText = i18next.t(
                                        datacontent.trim(),
                                    );
                                    ...
                                    currentValue.setAttribute(
                                        'data-content',
                                        i18next.t(datacontent.trim()),
                                    );
                                }
                            } else if (['P'].includes(currentValue.nodeName)) {
                                const textkey = currentValue.innerHTML
                                    .replace(/(\r\n|\n|\r)/gm, '')
                                    .trim();
                                ...
                                ['INPUT'].includes(currentValue.nodeName)
                            ) {
                                const textkey = currentValue
                                    .getAttribute('value')
                                    .trim();
                                ...
                                ['OPTGROUP'].includes(currentValue.nodeName)
                            ) {
                                const textkey = currentValue
                                    .getAttribute('label')
                                    .trim();

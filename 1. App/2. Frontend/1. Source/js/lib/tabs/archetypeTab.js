...
const KNOWN_SETS = [
    'SpeedSet', 'AttackSet', 'HealthSet', 'DefenseSet', 'CriticalSet',
    'ImmunitySet', 'HitSet', 'ResistSet', 'CounterSet', 'RevengeSet',
    'RageSet', 'LifestealSet', 'PenetrationSet', 'TorrentSet', 'PursuitSet',
    'InjurySet', 'DestructionSet', 'ReversalSet', 'RiposteSet', 'WarfareSet',
    'ProtectionSet', 'UnitySet',
];

const KNOWN_SUBSTATS = [
    'Speed', 'AttackPercent', 'Attack', 'HealthPercent', 'Health',
    'DefensePercent', 'Defense', 'CriticalHitChancePercent',
    'CriticalHitDamagePercent', 'EffectivenessPercent', 'EffectResistancePercent',
];

const KNOWN_NECKLACE = [
    'AttackPercent', 'HealthPercent', 'DefensePercent', 'Attack', 'Health',
    'Defense', 'CriticalHitChancePercent', 'CriticalHitDamagePercent',
];
const KNOWN_RING = [
    'AttackPercent', 'HealthPercent', 'DefensePercent', 'Attack', 'Health',
    'Defense', 'EffectivenessPercent', 'EffectResistancePercent',
];
const KNOWN_BOOTS = ['AttackPercent', 'HealthPercent', 'DefensePercent', 'Speed'];

// Valid substat pools per gear slot (mirrors E7 drop rules)
const SLOT_SUBSTATS = {
    Weapon:   ['AttackPercent', 'Health', 'HealthPercent', 'Speed', 'CriticalHitChancePercent', 'CriticalHitDamagePercent', 'EffectivenessPercent', 'EffectResistancePercent'],
    Helmet:   ['Attack', 'AttackPercent', 'Defense', 'DefensePercent', 'HealthPercent', 'Speed', 'CriticalHitChancePercent', 'CriticalHitDamagePercent', 'EffectivenessPercent', 'EffectResistancePercent'],
    Armor:    ['DefensePercent', 'Health', 'HealthPercent', 'Speed', 'CriticalHitChancePercent', 'CriticalHitDamagePercent', 'EffectivenessPercent', 'EffectResistancePercent'],
    Necklace: ['Attack', 'AttackPercent', 'Defense', 'DefensePercent', 'Health', 'HealthPercent', 'Speed', 'CriticalHitChancePercent', 'CriticalHitDamagePercent', 'EffectivenessPercent', 'EffectResistancePercent'],
    Ring:     ['Attack', 'AttackPercent', 'Defense', 'DefensePercent', 'Health', 'HealthPercent', 'Speed', 'CriticalHitChancePercent', 'CriticalHitDamagePercent', 'EffectivenessPercent', 'EffectResistancePercent'],
    Boots:    ['Attack', 'AttackPercent', 'Defense', 'DefensePercent', 'Health', 'HealthPercent', 'Speed', 'CriticalHitChancePercent', 'CriticalHitDamagePercent', 'EffectivenessPercent', 'EffectResistancePercent'],
};
...
function _label(val) {
    ...
    if (val && val.endsWith('Set')) return val.slice(0, -3);
    return val;
}
...
function _closeOpenPanel() {
    if (_openPanel) {
        ...
        if (_openPanel.parentNode) _openPanel.parentNode.removeChild(_openPanel);
        _openPanel = null;
    }
}
...
window.addEventListener('scroll', (e) => {
    ...
    if (_openPanel && _openPanel.contains(e.target)) return;
    _closeOpenPanel();
}, true);
...
function multiSelectTd(currentValues, options, onChange) {
    ...
    function updateTrigger(vals) {
        const text = (!vals || vals.length === 0) ? 'Any' : vals.map(_label).join(', ');
    ...
    function buildPanel(initVals) {
        ...
        options.forEach((opt) => {
            ...
            cb.dataset.val = opt;
            cb.checked = !!(initVals && initVals.includes(opt));
            cb.addEventListener('change', () => {
                if (cb.checked) anyCb.checked = false;
    ...
    buildPanel(initVals);
    updateTrigger(initVals && initVals.length ? initVals : null);
...
function textTd(value, onChange, wide) {
    ...
    input.type = 'text';
    input.value = value || '';
    input.className = 'archetype-cell-input' + (wide ? ' archetype-cell-wide' : '');
...
// Opens a panel listing each unique substat from S1-S6 with a numeric weight
// input. Default weight is 1.0 (neutral). Setting to 1.0 removes the key.
// ---------------------------------------------------------------------------
...
function statWeightsTd(archetype) {
    ...
    function refreshTrigger() {
        ...
        const anyVal = sw && sw._any != null ? sw._any : 1;
        const hasCustom = (anyVal !== 1) ||
    ...
    function buildPanel() {
        panel.innerHTML = '';
        const current = ArchetypeStore.getArchetypes().find((a) => a.id === archetype.id);
        const sw = (current && current.statWeights) ? current.statWeights : {};
        const anyVal = sw._any != null ? sw._any : 1;
        ...
        anyInp.addEventListener('input', () => {
            const v = parseFloat(anyInp.value);
            const fresh = ArchetypeStore.getArchetypes().find((a) => a.id === archetype.id);
            const newSw = { ...(fresh && fresh.statWeights ? fresh.statWeights : {}) };
            if (isNaN(v) || v === 1) {
                delete newSw._any;
            } else {
        ...
        const statList = [
            ...
            'Speed', 'CriticalHitChancePercent', 'CriticalHitDamagePercent',
        ];
        ...
        statList.forEach((stat) => {
            ...
            inp.value = sw[stat] != null ? sw[stat] : anyVal;

            inp.addEventListener('input', () => {
                const v = parseFloat(inp.value);
                const fresh = ArchetypeStore.getArchetypes().find((a) => a.id === archetype.id);
                const newSw = { ...(fresh && fresh.statWeights ? fresh.statWeights : {}) };
                const freshAny = newSw._any != null ? newSw._any : 1;
                if (isNaN(v) || v === freshAny) {
                    delete newSw[stat]; // inherit from Any
                } else {
...
function _onDragEnd(e) {
    ...
    document.querySelectorAll('.arch-row-over').forEach((el) => el.classList.remove('arch-row-over'));
}
...
function slotSubstatsTd(archetype) {
    ...
    function hasCustom() {
        ...
        return ss && Object.keys(ss).length > 0 &&
            Object.values(ss).some((arr) => Array.isArray(arr) && arr.some((v) => v !== null));
    }
    ...
    function buildPanel() {
        ...
        const fresh = ArchetypeStore.getArchetypes().find((a) => a.id === archetype.id) || archetype;
        ...
        SLOT_ORDER.forEach((slot) => {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                sel.addEventListener('change', () => {
                    const currentFresh = ArchetypeStore.getArchetypes().find((a) => a.id === archetype.id) || archetype;
                    ...
                    ArchetypeStore.updateArchetype(archetype.id, { slotSubstats: toStore });
                    refreshTrigger();
                });
...
function renderRow(archetype) {
    ...
    // Group
    tr.appendChild(textTd(archetype.group, (v) => {
        console.log('[ArchetypeTab] update group', archetype.id, '→', v);
        ArchetypeStore.updateArchetype(archetype.id, { group: v });
    }, false));

    // Name
    tr.appendChild(textTd(archetype.name || '', (v) => {
        console.log('[ArchetypeTab] update name', archetype.id, '→', v);
        ArchetypeStore.updateArchetype(archetype.id, { name: v });
    }, true));

    // 4pc Sets
    tr.appendChild(multiSelectTd(archetype.fourPieceSets, KNOWN_SETS, (v) => {
        console.log('[ArchetypeTab] update 4pcSets', archetype.id, '→', v);
        ArchetypeStore.updateArchetype(archetype.id, { fourPieceSets: v });
    }));

    // 2pc Sets
    tr.appendChild(multiSelectTd(archetype.twoPieceSets, KNOWN_SETS, (v) => {
        console.log('[ArchetypeTab] update 2pcSets', archetype.id, '→', v);
    ...
    function updateMain(slot, v) {
        console.log('[ArchetypeTab] update mainStats', archetype.id, slot, '→', v);
        const existing = ArchetypeStore.getArchetypes().find((a) => a.id === archetype.id);
        const ms = existing ? { ...(existing.mainStats || {}) } : {};
        ms[slot] = v;
        ArchetypeStore.updateArchetype(archetype.id, { mainStats: ms });
    }

    const necklaceVals = archetype.mainStats && archetype.mainStats.Necklace
        ? archetype.mainStats.Necklace.filter((x) => x !== null) : null;
    tr.appendChild(multiSelectTd(necklaceVals, KNOWN_NECKLACE, (v) => updateMain('Necklace', v)));

    const ringVals = archetype.mainStats && archetype.mainStats.Ring
        ? archetype.mainStats.Ring.filter((x) => x !== null) : null;
    tr.appendChild(multiSelectTd(ringVals, KNOWN_RING, (v) => updateMain('Ring', v)));

    const bootsVals = archetype.mainStats && archetype.mainStats.Boots
        ? archetype.mainStats.Boots.filter((x) => x !== null) : null;
    tr.appendChild(multiSelectTd(bootsVals, KNOWN_BOOTS, (v) => updateMain('Boots', v)));

    // Substats S1–S6
    const substats = archetype.substats || [null, null, null, null, null, null];
    for (let i = 0; i < 6; i++) {
        const idx = i;
        tr.appendChild(singleSelectTd(substats[i], (v) => {
            const existing = ArchetypeStore.getArchetypes().find((a) => a.id === archetype.id);
            const ss = existing
    ...
    // setBonus / mainBonus
    tr.appendChild(numTd(archetype.setBonus  ?? 1.25, (v) => ArchetypeStore.updateArchetype(archetype.id, { setBonus:  v })));
    tr.appendChild(numTd(archetype.mainBonus ?? 1.5,  (v) => ArchetypeStore.updateArchetype(archetype.id, { mainBonus: v })));
    ...
    delBtn.addEventListener('click', () => {
        if (!window.confirm('Delete this archetype?')) return;
        console.log('[ArchetypeTab] removeArchetype', archetype.id);
...
function _renderTable() {
    ...
    if (!tbody) { console.error('[ArchetypeTab] _renderTable: archetypeTableBody not found'); return; }
    const archetypes = ArchetypeStore.getArchetypes();
    console.log('[ArchetypeTab] _renderTable:', archetypes.length, 'archetypes', archetypes.map((a) => a.name));
    tbody.innerHTML = '';
...
const ArchetypeTab = {
    ...
    initialize() {
        ...
        console.log('[ArchetypeTab] initialize');

        _renderTable();

        document.getElementById('archetypeResetDefaults').addEventListener('click', () => {
            if (!window.confirm('Reset all archetypes to built-in defaults? This cannot be undone.')) return;
            console.log('[ArchetypeTab] resetToDefaults');
        ...
        document.getElementById('archetypeSave').addEventListener('click', () => {
            console.log('[ArchetypeTab] Save button clicked');
        ...
        document.getElementById('archetypeAddRow').addEventListener('click', () => {
            console.log('[ArchetypeTab] addRow');
            const newArchetype = {
        ...
        document.getElementById('archetypeExportJson').addEventListener('click', () => {
            console.log('[ArchetypeTab] exportJson clicked');
            const filename = _remote.dialog.showSaveDialogSync(_remote.getCurrentWindow(), {
                title: 'Export Archetypes',
                defaultPath: 'e7-archetypes.json',
                buttonLabel: 'Export',
                filters: [{ name: 'JSON', extensions: ['json'] }],
            });
            if (!filename) { console.log('[ArchetypeTab] exportJson cancelled'); return; }
            console.log('[ArchetypeTab] exportJson writing to:', filename);
            const json = JSON.stringify(ArchetypeStore.getArchetypes(), null, 2);
            try {
                _fs.writeFileSync(filename, json, 'utf8');
                console.log('[ArchetypeTab] exportJson write OK');
            } catch (e) {
                console.error('[ArchetypeTab] exportJson write FAILED:', e);
            }
        });

        document.getElementById('archetypeImportJson').addEventListener('click', () => {
            document.getElementById('archetypeImportFile').click();
        });

        document.getElementById('archetypeImportFile').addEventListener('change', (event) => {
            const file = event.target.files && event.target.files[0];
            if (!file) return;
            console.log('[ArchetypeTab] importJson reading file:', file.name);
            const reader = new FileReader();
            reader.onload = (e) => {
                try {
                    const parsed = JSON.parse(e.target.result);
                    if (!Array.isArray(parsed)) throw new Error('Expected a JSON array of archetypes.');
                    const isValid = parsed.length > 0 && parsed.every(
                        (a) => typeof a.id === 'string' && a.id.length > 0 && Array.isArray(a.substats),
                    );
                    if (!isValid) throw new Error('Each archetype must have a non-empty string id and a substats array.');
                    console.log('[ArchetypeTab] importJson loaded', parsed.length, 'archetypes');
                    ArchetypeStore.saveArchetypes(parsed);
                    _renderTable();
                } catch (err) {
                    console.error('[ArchetypeTab] importJson FAILED:', err);

...
const Importer = {
    addEventListener: () => {
        ...
        document
            .querySelectorAll('[id=loadIngameGearHeroesStart]')
            .forEach((x) =>
                x.addEventListener('click', async () => {
        ...
        document
            .querySelectorAll(
                '[id=loadIngameGearEnd], [id=loadIngameGearHeroesEnd]'
            )
            .forEach((x) =>
                x.addEventListener('click', async () => {
                    Scanner.end();
        ...
        document
            ...
            .addEventListener('click', async () => {
                const output =
                    document.getElementById('exportOutputText').value;

                if (!output || output.length === 0) {
                    Notifier.error(
                        'Nothing to export yet, please select a folder'
                    );
                    return;
                }
                ...
                if (output.includes('Reading screenshots in progress...')) {
                    Notifier.error(
                        'Screenshot reading in progress, please wait'
                    );
                    return;
                }
                ...
                const options = {
                    ...
                    defaultPath: Files.path(
                        `${Settings.getDefaultPath()}/gear.txt`
                    ),
                ...
                fs.writeFile(Files.path(filename), output, (err) => {
                    if (err) {
                        if (isEpermError(err)) {
                            ...
                            Notifier.error(
                                `${
                                    i18next.t('Unable to write file') + filename
                                } - ${err}`
                            );
                        }
                        return;
        ...
        document.querySelectorAll('.saveLoadFromGameExportBtn').forEach((x) =>
            x.addEventListener('click', async () => {
                const output = document.querySelector(
                    '.scanExportOutputText'
                ).value;
        ...
        document
            ...
            .addEventListener('click', async () => {
                ...
                try {
                    ...
                    Dialog.htmlError(
                        i18next.t(
                            "Error occurred while parsing gear. Check that you have <a href='https://github.com/fribbels/Fribbels-Epic-7-Optimizer#installing-the-app'>64-bit version of Java 8</a> installed and try again."
                        ) + e
                    );
                }
            });

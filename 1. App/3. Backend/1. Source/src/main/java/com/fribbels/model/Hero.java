        ...
        public DamageMultipliers getDamageMultipliers() {
                ...
                if (skillOptions == null) {
                        return DamageMultipliers.builder()
                                        ...
                                        .hitMulti(new Float[] { calculateHitMulti(0, skills.S1[0].name),
                                                        calculateHitMulti(1, skills.S2[0].name),
                                                        calculateHitMulti(2, skills.S3[0].name) })
                                        .support(new Float[] { calculateSupport(0, skills.S1[0].name),
                                                        calculateSupport(1, skills.S2[0].name),
                                                        calculateSupport(2, skills.S3[0].name) })
                                        .crit(new Float[] { calculateCrit(0, skills.S1[0].name),
                                                        calculateCrit(1, skills.S2[0].name),
                                                        calculateCrit(2, skills.S3[0].name) })
                                        .build();
                } else {
                        final SkillData s1 = Arrays.stream(skills.S1)
                                        .filter(x -> StringUtils.equals(x.name, skillOptions.getS1().skillEffect))
                                        .findFirst()
                                        .orElse(skills.S1[0]);
                        final SkillData s2 = Arrays.stream(skills.S2)
                                        .filter(x -> StringUtils.equals(x.name, skillOptions.getS2().skillEffect))
                                        .findFirst()
                                        .orElse(skills.S2[0]);
                        final SkillData s3 = Arrays.stream(skills.S3)
                                        .filter(x -> StringUtils.equals(x.name, skillOptions.getS3().skillEffect))
                                        .findFirst()
                                        .orElse(skills.S3[0]);
                        return DamageMultipliers.builder()
                                        ...
                                        .hitMulti(new Float[] {
                                                        calculateHitMulti(0, getSkillOptionsByIndex(0).skillEffect),
                                                        calculateHitMulti(1, getSkillOptionsByIndex(1).skillEffect),
                                                        calculateHitMulti(2, getSkillOptionsByIndex(2).skillEffect) })
                                        .support(new Float[] {
                                                        calculateSupport(0, getSkillOptionsByIndex(0).skillEffect),
                                                        calculateSupport(1, getSkillOptionsByIndex(1).skillEffect),
                                                        calculateSupport(2, getSkillOptionsByIndex(2).skillEffect) })
                                        .crit(new Float[] { calculateCrit(0, getSkillOptionsByIndex(0).skillEffect),
                                                        calculateCrit(1, getSkillOptionsByIndex(1).skillEffect),
                                                        calculateCrit(2, getSkillOptionsByIndex(2).skillEffect) })
                                        .build();
                }
        }

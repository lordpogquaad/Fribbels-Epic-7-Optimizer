...
public class StatCalculator {
    ...
    public HeroStats addAccumulatorArrsToHero(final HeroStats base,
        ...
        final int spd = (int) (base.spd + accs0[10] + accs1[10] + accs2[10] + accs3[10] + accs4[10] + accs5[10]
                ...
                + (sets[18] > 3 ? reversalSetBonus : 0) + hero.bonusSpeed + hero.aeiSpeed);

        // final int atk = (int) (((base.atk + mapAccumulatorArrsToFloat(0, accs) +
        // (sets[2] > 1 ? sets[2] / 4 * 0.35f * base.atk : 0) + base.atk *
        // hero.bonusAtkPercent / 100f) + hero.bonusAtk) * (1 +
        // base.bonusMaxAtkPercent/100f));
        // final int hp = (int) (((base.hp + mapAccumulatorArrsToFloat(1, accs) +
        // (sets[0] > 1 ? sets[0] / 2 * 0.15f * base.hp : 0) + base.hp *
        // hero.bonusHpPercent / 100f) + hero.bonusHp) * (1 +
        // base.bonusMaxHpPercent/100f));
        // final int def = (int) (((base.def + mapAccumulatorArrsToFloat(2, accs) +
        // (sets[1] > 1 ? sets[1] / 2 * 0.15f * base.def : 0) + base.def *
        // hero.bonusDefPercent / 100f) + hero.bonusDef) * (1 +
        // base.bonusMaxDefPercent/100f));
        // final float cr = (base.cr + mapAccumulatorArrsToFloat(6, accs) + (sets[4] > 1
        // ? sets[4] / 2 * 12 : 0)) + hero.bonusCr;
        // final int cd = (int) (base.cd + mapAccumulatorArrsToFloat(7, accs) + (sets[6]
        // > 1 ? sets[6] / 4 * 40 : 0)) + (int) hero.bonusCd;
        // final int eff = (int) (base.eff + mapAccumulatorArrsToFloat(8, accs) +
        // (sets[5] > 1 ? sets[5] / 2 * 20 : 0)) + (int) hero.bonusEff;
        // final int res = (int) (base.res + mapAccumulatorArrsToFloat(9, accs) +
        // (sets[9] > 1 ? sets[9] / 2 * 20 : 0)) + (int) hero.bonusRes;
        // final int spd = (int) (base.spd + mapAccumulatorArrsToFloat(10, accs) +
        // (sets[3] > 1 ? sets[3] / 4 * 0.25f * base.spd : 0) + (sets[14] > 1 ? sets[14]
        // / 4 * 0.1f * base.spd : 0)) + hero.bonusSpeed;
        ...
        final int cdCap = 350 + hero.cdCapBonus;
        if (cd > cdCap) {
            critDamage = cdCap / 100f;
        } else {
            critDamage = cd / 100f;
        }
        ...
        // final int s1 = (int)(((atk * multis.getAtkMods()[0] * multis.getRates()[0] +
        // getFlatMod(multis, 0, hp)) * getTypeMultiplier(multis, 0)) *
        // multis.getPows()[0] * multis.getMultis()[0]);
        // final int s2 = (int)(((atk * multis.getAtkMods()[1] * multis.getRates()[1] +
        // getFlatMod(multis, 1, hp)) * getTypeMultiplier(multis, 1)) *
        // multis.getPows()[1] * multis.getMultis()[1]);
        // final int s3 = (int)(((atk * multis.getAtkMods()[2] * multis.getRates()[2] +
        // getFlatMod(multis, 2, hp)) * getTypeMultiplier(multis, 2)) *
        // multis.getPows()[2] * multis.getMultis()[2]);
        //
        final int score = (int) (accs0[11] + accs1[11] + accs2[11] + accs3[11] + accs4[11] + accs5[11]);

        // final ArtifactStats artifactStats =
        ...
        final float bsAtk = (atk - base.atk - hero.artifactAttack - (sets[2] > 1 ? sets[2] / 4 * atkSetBonus : 0))
                / base.atk * 100;
        ...
        final float bsSpd = (spd - base.spd - (sets[3] > 3 ? speedSetBonus : 0) - (sets[14] > 3 ? revengeSetBonus : 0)
                - (sets[18] > 3 ? reversalSetBonus : 0));

        // hp: (row.hp - base.hp - artiHp - bonusSetMaxHp/100*base.hp -
        // bonusSetTorrent/100*base.hp) / base.hp * 100,
        // atk: (row.atk - base.atk - artiAtk - bonusSetAtt/100*base.atk) / base.atk *
        // 100,

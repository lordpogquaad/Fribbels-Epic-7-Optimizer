...
public class Hero {
        ...
        // Combat-readiness push as a fraction (e.g. 0.40 for a 40% push). Display-only:
        // drives the Final Speed / pSpd columns (effective speed = spd / (1 - crPush)).
        public float crPush;

        // Manual target speed the user assigns to rank the roster (fastest-first) in
        // the
        // Hero grid. Display/sort-only — never used in any stat or build calculation.
        public int targetSpeed;

        // Build Planner fields — all manual, display/sort/planning only (never used in
        // any
        // stat or build calculation). lowPickRate flags power-crept / low-usage units
        // so the
        // Hero grid can sink them to the bottom; usageRate is a manual usage % for
        // reference;
        // targetSets are the sets the user plans to build this hero in.
        public boolean lowPickRate;
        ...
        public DamageMultipliers getDamageMultipliers() {
                ...
                if (skills == null || skills.S1 == null || skills.S1.length == 0 || skills.S1[0].name == null) {
                ...
                if (skillOptions == null) {
                        return DamageMultipliers.builder()
                                        ...
                                        .targets(new Integer[] { skills.S1[0].targets, skills.S2[0].targets,
                                                        skills.S3[0].targets })
                                        .selfHpScaling(new Float[] { skills.S1[0].selfHpScaling,
                                                        skills.S2[0].selfHpScaling,
                                                        skills.S3[0].selfHpScaling })
                                        .selfAtkScaling(new Float[] { skills.S1[0].selfAtkScaling,
                                                        skills.S2[0].selfAtkScaling,
                                                        skills.S3[0].selfAtkScaling })
                                        .selfDefScaling(new Float[] { skills.S1[0].selfDefScaling,
                                                        skills.S2[0].selfDefScaling,
                                                        skills.S3[0].selfDefScaling })
                                        .selfSpdScaling(new Float[] { skills.S1[0].selfSpdScaling,
                                                        skills.S2[0].selfSpdScaling,
                                                        skills.S3[0].selfSpdScaling })
                                        .increasedValue(new Float[] { skills.S1[0].increasedValue,
                                                        skills.S2[0].increasedValue,
                                                        skills.S3[0].increasedValue })
                                        .extraSelfHpScaling(new Float[] { skills.S1[0].extraSelfHpScaling,
                                                        skills.S2[0].extraSelfHpScaling,
                                                        skills.S3[0].extraSelfHpScaling })
                                        .extraSelfDefScaling(new Float[] { skills.S1[0].extraSelfDefScaling,
                                                        skills.S2[0].extraSelfDefScaling,
                                                        skills.S3[0].extraSelfDefScaling })
                                        .extraSelfAtkScaling(new Float[] { skills.S1[0].extraSelfAtkScaling,
                                                        skills.S2[0].extraSelfAtkScaling,
                                                        skills.S3[0].extraSelfAtkScaling })
                                        .cdmgIncrease(new Float[] { skills.S1[0].cdmgIncrease,
                                                        skills.S2[0].cdmgIncrease,
                                        ...
                                        .hitMulti(new Float[] { calculateHitMulti(skills.S1[0].name),
                                                        calculateHitMulti(skills.S2[0].name),
                                                        calculateHitMulti(skills.S3[0].name) })
                                        .support(new Float[] { calculateSupport(skills.S1[0].name),
                                                        calculateSupport(skills.S2[0].name),
                                                        calculateSupport(skills.S3[0].name) })
                                        .crit(new Float[] { calculateCrit(skills.S1[0].name),
                                                        calculateCrit(skills.S2[0].name),
                                                        calculateCrit(skills.S3[0].name) })
                                        .build();
                } else {
                        final SkillData s1 = Arrays.stream(skills.S1)
                                        .filter(x -> Strings.CS.equals(x.name, skillOptions.getS1().skillEffect))
                                        .findFirst()
                                        .orElse(skills.S1[0]);
                        final SkillData s2 = Arrays.stream(skills.S2)
                                        .filter(x -> Strings.CS.equals(x.name, skillOptions.getS2().skillEffect))
                                        .findFirst()
                                        .orElse(skills.S2[0]);
                        final SkillData s3 = Arrays.stream(skills.S3)
                                        .filter(x -> Strings.CS.equals(x.name, skillOptions.getS3().skillEffect))
                                        .findFirst()
                                        .orElse(skills.S3[0]);
                        return DamageMultipliers.builder()
                                        ...
                                        .selfHpScaling(new Float[] { s1.selfHpScaling, s2.selfHpScaling,
                                                        s3.selfHpScaling })
                                        .selfAtkScaling(new Float[] { s1.selfAtkScaling, s2.selfAtkScaling,
                                                        s3.selfAtkScaling })
                                        .selfDefScaling(new Float[] { s1.selfDefScaling, s2.selfDefScaling,
                                                        s3.selfDefScaling })
                                        .selfSpdScaling(new Float[] { s1.selfSpdScaling, s2.selfSpdScaling,
                                                        s3.selfSpdScaling })
                                        .increasedValue(new Float[] { s1.increasedValue, s2.increasedValue,
                                                        s3.increasedValue })
                                        .extraSelfHpScaling(
                                                        new Float[] { s1.extraSelfHpScaling, s2.extraSelfHpScaling,
                                                                        s3.extraSelfHpScaling })
                                        .extraSelfDefScaling(
                                                        new Float[] { s1.extraSelfDefScaling, s2.extraSelfDefScaling,
                                                                        s3.extraSelfDefScaling })
                                        .extraSelfAtkScaling(
                                                        new Float[] { s1.extraSelfAtkScaling, s2.extraSelfAtkScaling,
                                                                        s3.extraSelfAtkScaling })
        ...
        private float calculateSupport(final String name) {
                if (Strings.CS.contains(name, "heal") ||
                                Strings.CS.contains(name, "barrier")) {
                        return 1f;
                }
                return 0f;
        }
        ...
        private float calculateCrit(final String name) {
                if (Strings.CS.contains(name, "crit")) {
                        return 1f;
                }
                return 0f;
        }
        ...
        private Float calculateHitMulti(final String name) {
                ...
                if (Strings.CS.contains(name, "crushing")) {
                        return 1.3f;
                }
                if (Strings.CS.contains(name, "normal")) {
                        return 1f;
                }
                if (Strings.CS.contains(name, "miss")) {
                        return 0.75f;
                }
                return 0f;
        }
        ...
        public void setModStats(final ModStatsRequest modStatsRequest) {
                ...
                this.maxModPieces = modStatsRequest.getMaxModPieces();
                ...
                this.modSlots = modStatsRequest.getModSlots();
                this.slotModConfig = modStatsRequest.getSlotModConfig();
        }
        ...
        public void setBonusStats(final BonusStatsRequest bonusStats) {
                ...
                this.crPush = bonusStats.getCrPush();
                this.targetSpeed = bonusStats.getTargetSpeed();
                this.lowPickRate = bonusStats.isLowPickRate();
                this.usageRate = bonusStats.getUsageRate();
                this.targetSets = bonusStats.getTargetSets();
                ...
                this.cdCapBonus = bonusStats.getCdCapBonus();
        ...
        public void setStats(final HeroStats heroStats) {
                ...
                this.hmcdmgs = heroStats.getHmcdmgs();
                this.dmcdmgs = heroStats.getDmcdmgs();
                this.hdmg = heroStats.getHdmg();
                this.hdmgs = heroStats.getHdmgs();
                this.ddmg = heroStats.getDdmg();
                this.ddmgs = heroStats.getDdmgs();
                ...
                this.customScore = heroStats.getCustomScore();

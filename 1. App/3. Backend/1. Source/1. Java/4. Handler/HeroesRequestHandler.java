...
public class HeroesRequestHandler extends RequestHandler implements HttpHandler {
    ...
    private static final Logger logger = Logger.getLogger(HeroesRequestHandler.class.getName());
    ...
    public synchronized void handle(final HttpExchange exchange) throws IOException {
        logger.info("===================== HeroesRequestHandler =====================");
        ...
        try {
            switch (path) {
                ...
                case "/heroes/setBonusStats":
                    ...
                    return;
                case "/heroes/setTargetSpeed":
                    final SetTargetSpeedRequest setTargetSpeedRequest = parseRequest(exchange,
                            SetTargetSpeedRequest.class);
                    sendResponse(exchange, setTargetSpeed(setTargetSpeedRequest));
                    return;
                case "/heroes/setBuildPlanner":
                    final SetBuildPlannerRequest setBuildPlannerRequest = parseRequest(exchange,
                            SetBuildPlannerRequest.class);
                    sendResponse(exchange, setBuildPlanner(setBuildPlannerRequest));
                    return;
                ...
                case "/heroes/removeBuild":
                    ...
                    return;
                case "/heroes/reorderAll":
                    final ReorderAllRequest reorderAllRequest = parseRequest(exchange, ReorderAllRequest.class);
                    sendResponse(exchange, reorderAll(reorderAllRequest));
                    return;
    ...
    public String editBuild(final BuildsRequest request) {
        ...
        final HeroStats matchingBuild = builds.stream()
                .filter(x -> Strings.CS.equals(x.getBuildHash(), buildHash))
                .findFirst()
                .orElse(null);
    ...
    public String reorderAll(final ReorderAllRequest request) {
        final List<String> ids = request.getIds();
        if (ids == null || ids.isEmpty())
            return "";

        final List<Hero> heroes = heroDb.getAllHeroes();
        final Map<String, Hero> byId = new HashMap<>();
        for (final Hero h : heroes)
            byId.put(h.getId(), h);

        final List<Hero> reordered = new ArrayList<>();
        // Place heroes in the requested order (byId.remove also dedupes repeats).
        for (final String id : ids) {
            final Hero h = byId.remove(id);
            if (h != null)
                reordered.add(h);
        }
        // Append any heroes not in the provided list, preserving their original order.
        for (final Hero h : heroes) {
            if (byId.containsKey(h.getId()))
                reordered.add(h);
        }

        heroDb.setHeroes(reordered);
        return "";
    }
    ...
    public String setTargetSpeed(final SetTargetSpeedRequest request) {
        final Hero hero = heroDb.getHeroById(request.getHeroId());
        if (hero == null)
            return "";

        hero.setTargetSpeed(request.getTargetSpeed());

        return "";
    }

    public String setBuildPlanner(final SetBuildPlannerRequest request) {
        final Hero hero = heroDb.getHeroById(request.getHeroId());
        if (hero == null)
            return "";

        if (request.getLowPickRate() != null)
            hero.setLowPickRate(request.getLowPickRate());
        if (request.getUsageRate() != null)
            hero.setUsageRate(request.getUsageRate());
        if (request.getTargetSpeed() != null)
            hero.setTargetSpeed(request.getTargetSpeed());
        ...
        try {
            ...
            for (final Hero hero : heroes) {
                try {
                    ...
                    logger.warning("Error computing stats for hero: " + hero.getName() + " - " + e);
            ...
            logger.severe("err" + e);
            return toJson(GetAllHeroesResponse.builder().heroes(new ArrayList<>()).build());
        }
    }
    ...
    private void addStatsToHero(final Hero hero, final boolean useReforgeStats) {
        ...
        final Item[] items = equipment.values().toArray(new Item[0]);
        ...
        final List<float[]> statAccumulators = equipment.values()
                ...
                .collect(Collectors.toList());
        final float[][] statAccumulatorArrs = statAccumulators.toArray(new float[0][]);
        ...
        final int weightedScore = equipment.values().stream().mapToInt(Item::getPriorityScore).sum();
        final HeroStats finalStats = statCalculator.addAccumulatorArrsToHero(baseStats, statAccumulatorArrs, setsArr,
                hero, upgrades, conversions, 0, priority, weightedScore);
    ...
    private void clearNullBuilds(final Hero hero, final HeroStats baseStats, final boolean useReforgeStats) {
        // baseStats is passed in by the caller (already fetched) — re-fetching here was
        // redundant
        // and would NPE if a future caller invoked this with a hero whose name has no
        // base stats.
    ...
    public boolean addStatsToBuild(final Hero hero, final HeroStats baseStats, final HeroStats build,
        ...
        for (int i = 0; i < 6; i++) {
            ...
            // Apply the mod to whichever AugmentedStats object buildStatAccumulatorArr will
            // actually read — reforgedStats when predicting reforges, augmentedStats
            // otherwise.
            // The mod value is already (pre-)reforged, so writing it into the matching
            // object
            // keeps the recomputed build stats correct on BOTH paths. Previously the mod
            // was
            // only ever applied to the reforged stats, so a non-reforge recalc
            // (useReforgeStats == false) silently dropped the mod's contribution.
            Item statItem = item;
            if (mod != null) {
                if (useReforgeStats && item.getReforgedStats() != null) {
                    final AugmentedStats cloned = item.getReforgedStats().withAttack(-1);
                    cloned.setAttack(item.getReforgedStats().getAttack());
                    mod.modifyAugmentedStats(cloned);
                    statItem = item.withReforgedStats(cloned);
                } else if (!useReforgeStats && item.getAugmentedStats() != null) {
                    final AugmentedStats cloned = item.getAugmentedStats().withAttack(-1);
                    cloned.setAttack(item.getAugmentedStats().getAttack());
                    mod.modifyAugmentedStats(cloned);
                    statItem = item.withAugmentedStats(cloned);
                }
            }
            statAccumulatorArrs[i] = statCalculator.buildStatAccumulatorArr(baseStats, statItem, useReforgeStats);
        }
        ...
        final int weightedScore = items.stream().mapToInt(Item::getPriorityScore).sum();
        final HeroStats finalStats = statCalculator.addAccumulatorArrsToHero(baseStats, statAccumulatorArrs, setsArr,
                hero, upgrades, conversions, 0, priority, weightedScore);
    ...
    public String removeHeroById(final IdRequest request) {
        ...
        if (hero.getEquipment() != null) {
            for (final Item previousItem : hero.getEquipment().values()) {
        ...
        final List<Hero> newHeroes = heroes.stream()
                .filter(x -> !Strings.CS.equals(x.getId(), request.getId()))
                .collect(Collectors.toList());
    ...
    public String toggleLockHeroById(final IdRequest request, final boolean locked) {
        ...
        if (hero.getEquipment() != null) {
            for (final Item item : hero.getEquipment().values()) {
                if (item == null) {
                    continue;
                }

...
**When assisting with any workflow change, prioritize in this order: (1) security issues in the diff, (2) correctness, (3) performance, (4) style.** For small, focused changes, apply only the guidance relevant to the affected section.

**Handling conflicts with best practices:** If the user explicitly requests a practice this guide discourages (e.g., 'just use @main for this action'), comply but include a brief one-line warning explaining the risk.
...
- **Deeper Dive:**
  ...
  - **Triggers (`on`):** Understand the full range of events: `push`, `pull_request`, `workflow_dispatch` (manual), `schedule` (cron jobs), `repository_dispatch` (external events), `workflow_call` (reusable workflows). **`pull_request_target` warning:** This trigger runs with write permissions and has access to secrets in the context of the base repo. Never check out or execute untrusted PR code (e.g., from a fork) in a `pull_request_target` workflow without strict isolation — doing so is a well-known critical supply chain vulnerability.
...
- **SHA Unknown:** If you do not know the current commit SHA for an action, do not invent one. Use a clearly marked placeholder like `<PIN-TO-LATEST-SHA>` and instruct the user to retrieve it from the action's repository releases page or via `gh api repos/{owner}/{repo}/git/ref/tags/{tag}`.
- **Existing Violations:** When editing an existing workflow that uses mutable references (e.g., `@v4`), flag them in your response and offer to convert to pinned SHAs, but do not modify them unless the user's request involves those lines.
...
- **Guidance for Copilot:**
  ...
  - Provide examples of how to configure the OIDC action for common cloud providers. Always pin to a full commit SHA. If you do not know the current SHA for an action, use `<PIN-TO-LATEST-SHA>` as a placeholder and instruct the user to retrieve it from the action's repository releases page.
...
```yaml
permissions:
  contents: read # GITHUB_TOKEN default varies by repo/org settings; always declare explicitly
...
### **8. Script Injection via Untrusted GitHub Context**

- **Principle:** Never interpolate untrusted GitHub context values directly into `run:` shell scripts.
- **Deeper Dive:**
  - **Untrusted Inputs:** Values such as `github.event.pull_request.title`, `github.event.issue.body`, `github.head_ref`, and commit messages are attacker-controlled and must never be embedded directly in shell commands via `${{ }}` expressions.
  - **Attack Vector:** An attacker can craft a PR title or branch name containing shell metacharacters that execute arbitrary code in your workflow.
  - **Safe Pattern:** Pass untrusted values through environment variables, which the shell treats as data rather than code.
- **Guidance for Copilot:**
  - Never emit `${{ github.event.pull_request.title }}`, `${{ github.event.issue.body }}`, `${{ github.head_ref }}`, or similar untrusted context values directly inside `run:` blocks.
  - Always use intermediate environment variables to pass such values safely:

```yaml
- name: Process PR title safely
  env:
    PR_TITLE: ${{ github.event.pull_request.title }}
  run: echo "PR title is: $PR_TITLE" # $PR_TITLE is treated as data, not code
```

- Treat any value sourced from user-controlled events (PR body, issue comments, branch names, commit messages) as untrusted input.
...
- **Guidance for Copilot:**
  ...
  - Default to `fail-fast: false` for test matrices to ensure all combinations run and produce comprehensive failure reports. Use `fail-fast: true` only when subsequent matrix runs are pointless after the first failure (e.g., a critical smoke-test matrix where any failure blocks everything).
...
- **Deeper Dive:**
  ...
  - **Code Coverage:** Integrate code coverage tools (e.g., Istanbul for JS, Coverage.py for Python, JaCoCo for Java) and enforce minimum coverage thresholds. Aim for 70–85% line coverage as a baseline, prioritizing critical business logic paths over chasing 100%.
...
**Strategy Selection — choose based on your requirements:**

- **Rolling** — Stateless services tolerant of briefly mixed-version state; simplest to operate.
- **Blue/Green** — When instant rollback is required and you can maintain two identical environments.
- **Canary** — When metrics-driven validation before full rollout is available; best for risk-sensitive changes.
- **Dark Launch / Feature Flags** — When decoupling deployment from feature release to enable targeted exposure.
- **A/B Testing** — When comparing user behavior across two feature variants with analytics.
...
**Typical Staged Rollout Percentages and Durations:**

| Strategy    | Stage 1                            | Stage 2               | Stage 3      | Full Rollout      |
| ----------- | ---------------------------------- | --------------------- | ------------ | ----------------- |
| Canary      | 1–5% (minutes–hours)               | 10–25% (hours)        | 50% (hours)  | 100%              |
| Blue/Green  | 0% → 100% (instant traffic switch) | —                     | —            | Immediate         |
| Rolling     | ~10% of instances                  | ~50%                  | ~90%         | 100%              |
| Dark Launch | 0% visible (100% deployed)         | Internal / beta users | Opt-in group | Toggle on for all |

...
- **INFO** — Finding suppressed by an inline accessibility suppression comment. Noted for transparency only.

**Analysis workflow:** Use a two-pass approach to manage the rule set systematically:

- **Pass 1 — Detect:** Check each element against all anti-pattern detection patterns and collect candidate violations.
- **Pass 2 — Filter:** Apply scope exclusions, the accessible-library allowlist, and suppression comment downgrades to the candidate set. Then deduplicate and format output.

**Scope:** Apply these rules only to files producing user-facing HTML output (`.html`, `.jsx`, `.tsx`, `.vue`, `.svelte`, `.astro`, component templates). Skip files matching these path patterns: `**/*.test.*`, `**/*.spec.*`, `**/__tests__/**`, `**/__mocks__/**`, `**/*.stories.*`, `**/fixtures/**`, and code blocks inside `.md`/`.mdx` files. Analyze all other matching files.

**Unparseable files:** If a file cannot be parsed or analyzed reliably (syntax errors, dynamic template generation, obfuscated code), output: `UNABLE_TO_ANALYZE: <file> — <reason>` and do not report findings for it.

**Detection patterns:** Detection patterns listed under each anti-pattern are heuristics, not strict regex matches. Apply them with the contextual exclusions noted in the prose description.

**Deduplication:** When multiple anti-patterns apply to the same code element (e.g., S8, K1, A5, NG1, VU1 all cover non-semantic clickable divs), report only one finding using this precedence: 1) Prefer the framework-specific rule (NG*, VU*, RX\*) when the file uses that framework. 2) Otherwise prefer the rule with highest severity. 3) On ties, prefer the more specific rule ID (e.g., K1 over S8 for keyboard issues).

**Manual verification:** When a rule requires runtime, visual, or external file inspection (contrast computation, caption file existence, focus order, reflow behavior), do not assert pass/fail. Report these in a separate `## Requires Manual Verification` section using format: `[Anti-pattern ID]: <what to check> at <file:line>. WCAG: <criterion>.` Do not assign a severity to these items.

**Suppression comments:** If the surrounding code includes an accessibility suppression comment (e.g., `// a11y-ignore`, `// eslint-disable-next-line jsx-a11y/*`), downgrade the finding to INFO and note the suppression rather than reporting as CRITICAL.

**Accessible component libraries:** Assume built-in accessibility handling and do not flag missing focus traps, ARIA roles, or keyboard handlers for the following library primitives: Angular Material (`@angular/material`) — `MatDialog`, `MatMenu`, `MatSelect`; Radix UI (`@radix-ui/*`) — all primitives; Headless UI (`@headlessui/*`) — all primitives; shadcn/ui (`@/components/ui/*`) — all primitives; React Aria (`@react-aria/*`, `react-aria-components`) — all primitives. For any other library not in this list, still report findings.

**Output format:** Report each finding as: `[Severity] [Anti-pattern ID]: <description> at <file:line>. WCAG: <criterion>. Fix: <code snippet>`. Group by severity (CRITICAL first, then IMPORTANT, then SUGGESTION). INFO findings appear in a final section after SUGGESTION, formatted: `[INFO] [Anti-pattern ID]: <description> (suppressed by <comment>) at <file:line>`. If no violations are found, output exactly: `No accessibility issues detected in <file/scope>.` Do not invent findings.
...
- **Severity**: SUGGESTION
- **Detection**: More than one `<h1>` where the additional `<h1>` elements are not inside an `<article>` or `<section>` that contains an `<h1>`–`<h6>` as a direct or nested child within its first heading element (i.e., has its own heading context)
...
Required per role: `tab` needs `aria-selected`; `combobox` needs `aria-expanded`/`aria-controls`; `slider` needs `aria-valuemin`/`aria-valuemax`/`aria-valuenow`; `checkbox` needs `aria-checked`. Dynamic bindings (`aria-selected={...}`, `[attr.aria-checked]="..."`, `:aria-checked`) satisfy the requirement. Only flag if the attribute is entirely absent from the element.
...
- **Severity**: IMPORTANT
- **Detection**: A visible `*` indicator (e.g., in a `<label>`) on a field that lacks the HTML `required` attribute or `aria-required="true"`
...
- **Severity**: CRITICAL
- **Detection**: Elements with class names containing `error`, `success`, or `warning` that lack an accompanying icon, text label, or `aria-label` indicating state
- **WCAG**: 1.4.1 (A)
...
- **Severity**: IMPORTANT
- **Detection**: CSS rules with `width` values >= 320px in `px` units on layout containers (`main`, `section`, `article`, `.container`, `.wrapper`). Ignore icons, avatars, and components < 320px.
- **WCAG**: 1.4.10 (AA)
...
- **WCAG**: 2.3.3 (AAA) — _beyond WCAG 2.2 AA; intentionally included as a best-practice SUGGESTION. Always report as SUGGESTION regardless of the AA scope target._

Gate non-essential animations behind `prefers-reduced-motion` so users who request less motion are not forced to experience interaction-triggered effects. This is a best-practice recommendation that exceeds WCAG 2.2 AA requirements.
...
## Framework-Specific: Other Frameworks

For frameworks not explicitly covered (Svelte, Solid, Astro, Web Components, Qwik, etc.), apply the Semantic HTML (S1–S8), ARIA (A1–A8), Keyboard (K1–K7), Form (F1–F6), Visual (V1–V5), and Media (D1–D4) anti-patterns. Map framework-specific event handlers to the generic rules: `on:click` (Svelte) → K1/S8; `onClick` (Solid) → K1/S8; and equivalent event binding syntax in other frameworks → K1/S8.

**Web Components (Shadow DOM):** Additionally verify that slotted interactive content retains accessible names across shadow boundaries (e.g., `<slot>` content labeled via `aria-labelledby` pointing to a slotted element must be confirmed to resolve correctly), and that focus is managed across shadow roots when dialogs or overlays span component boundaries.
